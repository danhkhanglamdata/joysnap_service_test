...
### /codebase [scope]
Explore, index, and load codebase context.

```bash
/codebase                     # Index entire codebase
/codebase src/api/            # Load specific area
/codebase "where is auth?"    # Search and answer questions
```

**When to use**: Code exploration and codebase understanding
...
### /qa [target]
Run unified quality assurance analysis (code quality, security, and tests).

```bash
/qa pr
```

**When to use**: Reviewing changes with the current QA workflow
